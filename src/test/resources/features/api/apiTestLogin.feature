Feature: API Test Scenarios
  ...
  @api
  Scenario: Create a New User using Map and using Serialization
  ...
  @api
  Scenario: Create a New User using POJO Class and using Serialization
    Given I add the headers "Content-Type", "application/json" and "Accept", "application/json"
  ...
  @api
  Scenario: Create a New User and Verify Login with multiple endpoints
    Given I add the headers "Content-Type", "application/json" and "Accept", "application/json"
    And I create a fake credentials and request in a map
    When I POST newly created user
    Then I verify the status code should be not greater than 300
